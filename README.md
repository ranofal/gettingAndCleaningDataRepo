...
Function myGrep: return True if a reqular expression regularExp in the column name feature
...
# Prepare 1 for Step 1: 
download the the zip file that contains all the data, then try to unzip it.\
...
## Subset the measurement for the mean and standard deviation
   ...
   where .* is any string, then the string "mean()" then again any string .* after that e.g.\
   tBodyAcc-mean()-X\
...
So need to convert both columns to have the same name. 
...
Merge the 2 datatable by activityNumber, then set the keys subject, activityNumber, and activityName,create a new table from short_wide to tall_narrow
